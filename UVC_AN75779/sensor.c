        ...
        void)
{
    ...
    SensorRead(SENSOR_ADDR_RD,0x00,0x00,0x04,buf);
...
/*
   Get the current contrast setting from the sensor.
 */
...
/*
   Get the current Hue setting from the sensor.
 */
        ...
        void)
{
    ...
    SensorRead (SENSOR_ADDR_RD, 0x04, 0x00,0x04, buf);
    return (uint8_t)buf[1];
}


/*
   Get the current saturation setting from the sensor.
 */
        ...
        void)
{
    ...
    SensorRead (SENSOR_ADDR_RD, 0x08, 0x00,0x04, buf);
    return (uint8_t)buf[1];
}

/*
   Get the current sharpness setting from the sensor.
 */
        ...
        void)
{
    ...
    SensorRead (SENSOR_ADDR_RD, 0x10, 0x00,0x04, buf);
    return (uint8_t)buf[1];
}

/*
   Get the current Gamma setting from the sensor.
 */
        ...
        void)
{
    ...
    SensorRead (SENSOR_ADDR_RD, 0x14, 0x00,0x04, buf);
    return (uint8_t)buf[1];
}

/*
   Get the current white balance setting from the sensor.
 */

uint8_t
SensorGetWhiteBalance(
        void)
{
    uint8_t buf[2];
...
/*
   Get the current Backlight compensation setting from the sensor.
 */

uint8_t
SensorGetBacklightComp(
        void)
{
    uint8_t buf[2];
...
/*
   Get the current Gain setting from the sensor.
 */
        ...
        uint8_t input)
{
	...
	a= input * 105;
	 ...
	 buf1[0]=0x00;
	 buf1[1]=0x00;
	 buf1[2]=(uint8_t)((a&0xff00)>>8);
	 buf1[3]=(uint8_t)((a&0x00ff));
    SensorWrite (SENSOR_ADDR_WR, 0x0C, 0x00, 4, buf1);
}


/*
   Update the contrast setting for the sensor...h1
 */
        ...
        uint8_t input)
{
	uint8_t ad_buf[4];
		...
		ad_buf[2]=0x20 + input ;
		ad_buf[3]=0x00 + input;
    SensorWrite (SENSOR_ADDR_WR, 0x00, 0x00, 4, ad_buf);
}

/*
   Update the Hue setting for the sensor....h2
 */
        ...
        uint8_t input)
{
	...
	uint8_t ad1_buf[4];
		...
		ad1_buf[0]=0x00;
		ad1_buf[1]=0x01;
		ad1_buf[2]=0x00 + input ;
		ad1_buf[3]=0x20 + input;
    SensorWrite (SENSOR_ADDR_WR, 0x04, 0x00, 4, ad1_buf);
}


/*
   Update the saturation setting for the sensor...rg
 */
        ...
        uint8_t input)
{
	...
	uint8_t ad2_buf[4];
		...
		ad2_buf[0]=0x00;
		ad2_buf[1]=0x01;
		ad2_buf[2]=0x00 + input ;
		ad2_buf[3]=0x0c + input;
    SensorWrite (SENSOR_ADDR_WR, 0x08, 0x00, 4, ad2_buf);
}

/*
   Update the Sharpness setting for the sensor....led on/off
 */
        ...
        uint8_t input)
{
	...
	if (input==1)
	{
		...
		LED_buf[2]=0x01;
		LED_buf[3]=0x01;
		SensorWrite(SENSOR_ADDR_WR,0x10,0x00,4,LED_buf); //LED
	}
...
/*
   Update the Gamma setting for the sensor......exposure
 */
...
SensorSetGamma (
        uint8_t input)
{
    SensorWrite (SENSOR_ADDR_WR, 0x14, 0x00, 4, input);
}


/*
   Update the WhiteBalance setting for the sensor.
 */

void
SensorSetWhiteBalance (
        uint8_t input)
{
...
/*
   Update the Backlight Compensation setting for the sensor.
 */

void
SensorSetBacklightComp (
        uint8_t input)
{
    SensorWrite2B (SENSOR_ADDR_WR, 0xCC, 0x0A, 0x00, input);
}
...
/*
   Update the Gain setting for the sensor.
 */
